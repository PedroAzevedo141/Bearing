...
- **Tela de Ajustes** (`/ajustes`, atalho na aba Rotação) com seletor de tema **Claro / Escuro / Automático**, persistido em `settings` (**migration v10**) e reaplicado na abertura. O acesso ao Backup passou a ficar dentro dela.
- **Tema escuro**, seguindo a preferência do sistema quando o usuário escolhe "Automático". A paleta virou variáveis CSS em `global.css` (é lá que claro e escuro se separam) e o `tailwind.config.js` passou a apontar para elas em vez de duplicar valores; `useThemeColors()` cobre o que passa cor via `style` em vez de `className`. Tokens novos: `spotlight` (cards de saldo) e `track` (barra de progresso). Ver ADR-0005 revisado.
- **Cobertura de testes da camada de queries** (`src/db/queries/`): transações, orçamentos, assinaturas, parcelas e backup, rodando o SQL real contra um SQLite real em memória (`node:sqlite`, sem dependência nova) com as migrations reais aplicadas — ver ADR-0011. A suíte foi de 83 para 145 testes.
...
- Botões brancos sobre os cards de destaque (aba Assistente e importação de PDF) ficavam com texto invisível no tema escuro: o rótulo usava `ink`, que é quase branco nesse tema. Passaram a usar `spotlight`, escuro nos dois. Junto disso, o novo token `onPrimary` corrige o contraste sobre `primary` — no escuro `primary` é um azul claro, e texto branco sobre ele fica ilegível (afetava também o ícone de enviar do chat e os botões `contained` do Paper).
